...
micrite.security.userList.SearchPanel = Ext.extend(micrite.ComplexEditorGrid, {
    ...
    var config = {
            ...
            columnsArray: [[
                {header: this.fullName, width: 80, sortable: true, dataIndex: 'fullname',
                 editor: new Ext.form.TextField({allowBlank: false})},
                {header: this.userName, width: 80, sortable: true, dataIndex: 'loginname'},
                {header: this.phone, width: 50, sortable: true, dataIndex: 'phone',
                  editor: new Ext.form.TextField({})},
                {header: this.unit, width: 60, sortable: true, dataIndex: 'unit',
                  editor: new Ext.form.TextField({})},
                {header: this.position, width: 60, sortable: true, dataIndex: 'position',
                   editor: new Ext.form.TextField({})},
                {header: this.grade, width: 60, sortable: true, dataIndex: 'grade',
                   editor: new Ext.form.TextField({})},
                {header: this.email, width: 60, sortable: true, dataIndex: 'emailaddress',
                   editor: new Ext.form.TextField({})},
                 // 状态不用下拉框，便于批量操作
                {header: this.enabled, width: 20, sortable: true, dataIndex: 'enabled'},
                sm
            ]],
            tbarActions : [{
                text:this.addUser,
            ...
            if (buttonId == 'yes') {
                micrite.util.ajaxRequest({
                    ...
                    params:{'user.id':user.id,
                            ...
                            'user.unit':user.get('unit'),
                            'user.position':user.get('position'),
                            'user.grade':user.get('grade')},
                    success:function(r,o){
