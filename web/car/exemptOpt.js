...
Ext.namespace('micrite.car.exemptOpt');
micrite.car.exemptOpt.SearchPanel = Ext.extend(micrite.ComplexGrid, {	
...
//            height:350,
            ...
            buttons: [{
                ...
                handler: function(){
                	if(!Ext.getCmp("exemptOpt-form").getForm().isValid())return;
                    Ext.getCmp("exemptOpt-form").getForm().submit({
                        url: 'car/exemptPunish.action',
                        method: 'POST',
                        disabled:true,
  ...
  // 通过checkbox选择或反选时触发选区轮换的一个制定选区模型
	...
	function showExpiredBackWindowFn(){						
	     if(mainGridPanelSM.getCount()==1){
	     	...
	     	var theForm=Ext.getCmp("exemptOpt-form").getForm();
	...
	var config = {
		...
		micrite.car.exemptOpt.SearchPanel.superclass.initComponent.apply(this, arguments);
    } // eo function initComponent
    
}); // eo extend

try {exemptOptLocale();} catch (e) {}
Ext.onReady(function() {
	Ext.QuickTips.init();	
	var formPanel = new micrite.car.exemptOpt.SearchPanel();	
	if (mainPanel) {
